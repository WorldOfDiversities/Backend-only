...
class PaymentViewSet(viewsets.ModelViewSet):
	...
	@action(detail=True, methods=['post'], url_path='finalize', permission_classes=[IsStaffRole])
	def finalize_payment(self, request, pk=None):
		"""Mark a pending payment as completed after verification.
		
		POST /payments/{id}/finalize/
		Body: {"verified": true}
		
		Used after Paystack payment is confirmed.
		"""
		...
		payment_id = pk
		try:
			result = verify_and_complete_payment(payment_id=int(payment_id))
			return Response(
				{
